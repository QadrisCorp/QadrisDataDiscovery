"""TDCC OpenAPI discovery — auto-discover all endpoints from OpenAPI 3.0 spec.

集保中心(臺灣集中保管結算所)OpenAPI 為 3.0.x,path 已含 ``/v1/...``,
故資料 URL 直接為 ``tdcc_openapi_base + path``(勿再接 base path)。
所有 endpoint 皆為最新一期 snapshot,標 ``supports_history=False``。
"""
...
# OpenAPI 3.0 spec(非 swagger 2.0):回傳完整 api-docs JSON
SWAGGER_URL = "https://openapi.tdcc.com.tw/tdcc-opendata-api-docs"


def discover(*, settings: DiscoverySettings | None = None) -> list[EndpointInfo]:
    """Discover all TDCC OpenAPI endpoints from the api-docs spec."""
    if settings is None:
    ...
    logger.info("Fetching TDCC OpenAPI api-docs ...")
    try:
        swagger, status = fetch_json(SWAGGER_URL, session=session, settings=settings)
    except FetchError:
        logger.error("Failed to fetch TDCC api-docs")
        return endpoints

    if not swagger:
        logger.error("Empty TDCC api-docs (status=%s)", status)
        return endpoints
    ...
    for path, methods in paths.items():
        for method, spec in methods.items():
            ...
            # OpenAPI 3.0:path 已含 /v1,直接接 base(無 basePath)
            url = f"{settings.tdcc_openapi_base}{path}"
...
def main() -> None:
    """Run TDCC OpenAPI discovery and save to database."""
    ...
    logger.info("TDCC OpenAPI: %d endpoints saved to DB", len(endpoints))
