"""TPEx OpenAPI discovery — auto-discover all endpoints from swagger spec."""
...
def discover(*, settings: DiscoverySettings | None = None) -> list[EndpointInfo]:
    ...
    logger.info("Found %d endpoints (basePath=%s)", len(paths), base_path)
    ...
    for path, methods in paths.items():
        for method, spec in methods.items():
            ...
            url = f"{settings.tpex_openapi_base}{path}"
                ...
                if data is not None:
                    if isinstance(data, list):
                        ...
                        if data and isinstance(data[0], dict):
                            ep.sample_fields = list(data[0].keys())
                            sample_path = (
